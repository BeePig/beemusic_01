...
public class SynchronizeRepository implements SynchronizeContract {
    ...
    private SynchronizeRepository() {
        mContentResolver = BeeApplication.getInstant().getContentResolver();
    ...
    private Song getSong(Cursor cursor) {
        ...
        nameSong = cursor.getString(cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.TITLE));
        duration = cursor.getInt(cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.DURATION));
        linkSong = cursor.getString(cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.DATA));
        genre = getGenreFromMediaStore(idSong);
        return new Song(idSong, nameSong, linkSong, IS_NON_FAVORITE, TYPE_MEDIA, genre, duration);
    }

    private Album getAlbum(Cursor cursor) {
        String nameALbum =
                cursor.getString(cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.ALBUM));
        int idAlbum = cursor.getInt(cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.ALBUM_ID));
    ...
    public Cursor getDelCursor(Cursor currentMediaCursor) {
        ...
        while (currentMediaCursor.moveToNext()) {
            allSong += currentMediaCursor.getString(
                    currentMediaCursor.getColumnIndex(MediaStore.Audio.Media._ID)) + ",";
        }
        ...
        String selection = SongSourceContract.SongEntry.COLUMN_ID_SONG
                ...
                + allSong
                + ") and "
                + SongSourceContract.SongEntry.COLUMN_TYPE
                + " = ?";
        return mSongHandler.getCursor(selection, new String[] {
                String.valueOf(TYPE_MEDIA)
        });
    }
    ...
    public Cursor getAddCursor(Cursor currentMediaCurrsor) {
        ...
        while (appCursor.moveToNext()) {
            allSong += appCursor.getString(
                    appCursor.getColumnIndex(SongSourceContract.SongEntry.COLUMN_ID_SONG)) + ",";
        }
        allSong = allSong.substring(0, allSong.length() - 1);
        appCursor.close();
